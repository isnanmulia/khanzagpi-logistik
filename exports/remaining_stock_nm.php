<?php
    $title = "Sisa Stok Barang Non Medis";
    ...
    $lstitem .= "</tr>";
    $qGetItem = "SELECT kode_brng, nama_brng, satuan FROM ipsrsbarang B INNER JOIN kodesatuan S ON B.kode_sat=S.kode_sat ORDER BY kode_brng";
    $getItem = mysqli_query($connect_app, $qGetItem);
    $i = 0;
    while ($data = mysqli_fetch_assoc($getItem)) {
        $total = 0;
        $lstItem .= "<tr><td>" . ++$i . "</td><td class='str'>" . $data["kode_brng"] . "</td><td>" . $data["nama_brng"] . "</td><td>" . $data["satuan"] . "</td>";
        $qGetStok = "SELECT " . $pivotBangsal . " FROM gpi_gudangbarangipsrs WHERE kode_brng='" . $data["kode_brng"] . "'";
