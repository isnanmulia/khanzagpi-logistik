    ...
    if (isset($_POST["form_submit"])) {
        ...
        foreach ($items as $item) {
            ...
            if ($_POST["hdn_editable_" . $item] == "1") {
                ...
                $qInsertRcpDtl = "INSERT INTO ipsrsdetailpesan (no_faktur, kode_brng, no_pemesanan, kode_sat, kode_satbesar, isi, jumlah, harga, subtotal, dis, dis2, besardis, total) VALUES ('" . $_POST["hdn_id"] . "', '" . $itemx[0] . "', '" . $itemx[1] . "', '" . $_POST["hdn_sk_" . $item] . "', '" . $_POST["hdn_sb_" . $item] . "', " . $_POST["hdn_fk_" . $item] . ", " . $diterima . ", " . $hpesan . ", " . $_POST["hdn_subtotal_" . $item] . ", " . $_POST["hdn_disc1_" . $item] . ", " . $_POST["hdn_disc2_" . $item] . ", " . $_POST["hdn_ndisc_" . $item] . ", " . $_POST["hdn_total_" . $item] . ")";
                ...
                if ($hpesan > $price["harga_sat_besar"]) {
                    ...
                    $qInsertPriceHistory = "INSERT INTO gpi_riwayat_harga_nonmedis (no_ref, kode_brng, tanggal_efektif, harga_sat_kecil, harga_sat_besar, dibuat_oleh, dibuat_pada) VALUES ('" . $_POST["hdn_id"] . "', '" . $itemx[0] . "', '" . $today . "', " . $basePriceVat . ", " . $hpesan . ", '" . $_SESSION["USER"]["USERNAME"] . "', now())";
            ...
            if ($status) {
                ...
                logError("edit_po_reception_nm.php", "updatePODtl:" . $updatePODtl . "|updatePO:" . $updatePO . "|insertRcpDtl:" . $insertRcpDtl . "|insertRiwayat:" . $insertRiwayat . "|gudangBarang:" . $gudangBarang . "|insertPriceHistory:" . $insertPriceHistory . "|updatePriceMaster:" . $updatePriceMaster);
        ...
        $RcpNo = $_GET["id"];
        $qGetRcp = "SELECT no_faktur_supplier, PS.kode_suplier, nama_suplier, tgl_pesan, tgl_faktur, tgl_tempo, total1, potongan, total2, ppn, meterai, tagihan, PS.status, catatan, nama, nm_bangsal FROM ipsrspemesanan PS INNER JOIN ipsrssuplier S ON PS.kode_suplier=S.kode_suplier INNER JOIN petugas P ON PS.nip=P.nip INNER JOIN bangsal B ON PS.kd_bangsal=B.kd_bangsal WHERE no_faktur='" . $RcpNo . "'";
        $getRcp = mysqli_query($connect_app, $qGetRcp);
        $Rcp = mysqli_fetch_assoc($getRcp);
        $qGetRcpDtl = "SELECT DPS.kode_brng, nama_brng, DPS.no_pemesanan, Concat(DPS.kode_brng,'_',DPS.no_pemesanan,'_',DSPN.jumlah) AS kode, DPS.kode_sat, DSPN.jumlah AS jumlah_dipesan, SK.satuan AS satuan_kecil, DPS.kode_satbesar, SB.satuan AS satuan_besar, DPS.isi, DPS.jumlah AS jumlah_diterima, DPS.harga, DPS.subtotal, DPS.dis, DPS.dis2, DPS.besardis, DPS.total FROM ipsrsdetailpesan DPS INNER JOIN ipsrsbarang B ON DPS.kode_brng=B.kode_brng INNER JOIN kodesatuan SK ON DPS.kode_sat=SK.kode_sat INNER JOIN kodesatuan SB ON DPS.kode_satbesar=SB.kode_sat INNER JOIN detail_surat_pemesanan_non_medis DSPN ON DPS.kode_brng=DSPN.kode_brng AND DPS.no_pemesanan=DSPN.no_pemesanan WHERE no_faktur='" . $RcpNo . "'";
        ...
        while ($data = mysqli_fetch_assoc($getRcpDtl)) {
            ...
            $lstRcpDtl .= "<tr id='tr_" . $data["kode"] . "'><td>" . $data["kode_brng"] . "</td><td>" . str_replace('"', '\"', $data["nama_brng"]) . "</td><td>" . $data["satuan_kecil"] . "<input type='hidden' id='hdn_sk_" . $data["kode"] . "' name='hdn_sk_" . $data["kode"] . "' value='" . $data["kode_sat"] . "'></td><td>" . $data["satuan_besar"] . "<input type='hidden' id='hdn_sb_" . $data["kode"] . "' name='hdn_sb_" . $data["kode"] . "' value='" . $data["kode_satbesar"] . "'></td><td class='num-input'>" . number_format($data["isi"], 0, ",", ".") . "<input type='hidden' id='hdn_fk_" . $data["kode"] . "' name='hdn_fk_" . $data["kode"] . "' value='" . $data["isi"] . "'></td><td class='num-input'>" . number_format($data["jumlah_dipesan"], 0, ",", ".") . " " . $data["satuan_besar"] . "<input type='hidden' id='hdn_jml_" . $data["kode"] . "' name='hdn_jml_" . $data["kode"] . "' value='" . $data["jumlah_dipesan"] . "'></td><td class='num-input'>" . number_format($data["harga"], 0, ",", ".") . "<input type='hidden' id='inp_hpesan_" . $data["kode"] . "' name='inp_hpesan_" . $data["kode"] . "' value='" . $data["harga"] . "'></td><td class='num-input'><input type='hidden' id='hdn_sdhditerima_" . $data["kode"] . "' name='hdn_sdhditerima_" . $data["kode"] . "' value='" . $data["jumlah_diterima"] . "'></td><td colspan='2' class='num-input'>" . number_format($data["jumlah_diterima"], 0, ",", ".") . " " . $data["satuan_besar"] . "<input type='hidden' id='inp_diterima_" . $data["kode"] . "' name='inp_diterima_" . $data["kode"] . "' value='" . $data["jumlah_diterima"] . "'></td><td class='num-input'>" . number_format($data["subtotal"], 0, ",", ".") . "</td><td class='num-input'>" . str_replace(".", ",", $data["dis"]) . "</td><td class='num-input'>" . str_replace(".", ",", $data["dis2"]) . "</td><td class='num-input'>" . number_format($data["besardis"], 0, ",", ".") . "</td><td class='num-input td-total'>" . number_format($data["total"], 0, ",", ".") . "</td><td>" . $data["no_pemesanan"] . "</td><td><input type='hidden' id='hdn_editable_" . $data["kode"] . "' name='hdn_editable_" . $data["kode"] . "' value='0'></td></tr>";
        }
    }
?>
            ...
            <td colspan="3">
                ...
                <div id="div-scroll" style="height: 300px; overflow: scroll;">
                    ...
                    <table class="table table-hover" id="tbl_listItems">
                        <thead>
                            <tr>
                                <th style="min-width: 90px">Kode</th><th style="min-width: 200px">Nama Barang</th><th style="min-width: 55px" title="Satuan Kecil">SK</th><th style="min-width: 55px" title="Satuan Besar">SB</th><th style="min-width: 55px">Isi</th><th style="min-width: 90px">Jml. Pesan</th><th style="min-width: 110px">Harga Per SB</th><th style="min-width: 80px">Sudah Diterima</th><th colspan="2" style="min-width: 80px">Jumlah Datang</th><th style="min-width: 90px">Subtotal</th><th style="min-width: 70px">Diskon 1 (%)</th><th style="min-width: 70px">Diskon 2 (%)</th><th style="min-width: 90px">Total Diskon</th><th style="min-width: 90px; background-color: #EEE">Total</th><th style="min-width: 115px">No. Pemesanan</th><th style="min-width: 45px">Aksi</th>
                            </tr>
                        </thead>
