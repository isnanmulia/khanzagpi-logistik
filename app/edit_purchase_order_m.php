<?php
    ...
    if (isset($_POST["form_submit"])) {
        ...
        $qUpdatePO = "UPDATE surat_pemesanan_medis SET kode_suplier='" . $_POST["hdn_supplier"] . "', nip='" . $_POST["hdn_user"] . "', total1=" . $_POST["hdn_total1"] . ", potongan=" . $_POST["hdn_potongan"] . ", total2=" . $_POST["hdn_total2"] . ", ppn=" . $_POST["hdn_ppn"] . ", meterai=" . str_replace(".", "", $_POST["inp_meterai"]) . ", tagihan=" . $_POST["hdn_jml_tagihan"] . ", catatan='" . $_POST["inp_catatan"] . "' WHERE no_pemesanan='" . $_POST["hdn_id"] . "' AND tanggal='" . $_POST["hdn_date"] . "'";
        $updatePO = mysqli_query($connect_app, $qUpdatePO);
        saveToTracker($qUpdatePO, $connect_app);
        $updatePRD = TRUE; $updatePRDtlD = TRUE; $updatePRA = TRUE; $updatePRDtlA = TRUE; $deletePODtl = TRUE; $insertPODtl = TRUE; $updatePODtl = TRUE;
        if (strlen($_POST["hdn_delitems"])) {
            $delitems = explode(",", $_POST["hdn_delitems"]);
            foreach ($delitems as $item) {
                $itemx = explode("_", $item);
                if (isset($itemx[1])) {
                    $qUpdatePRDtlD = "UPDATE detail_pengajuan_barang_medis SET status='Proses Pengajuan' WHERE no_pengajuan='" . $itemx[1] . "' AND kode_brng='" . $itemx[0] . "'";
                    $updatePRDtlD = mysqli_query($connect_app, $qUpdatePRDtlD);
                    saveToTracker($qUpdatePRDtlD, $connect_app);
                    $qUpdatePRD = "UPDATE pengajuan_barang_medis SET status='Pengajuan' WHERE no_pengajuan='" . $itemx[1] . "'";
                    $updatePRD = mysqli_query($connect_app, $qUpdatePRD);
                    saveToTracker($qUpdatePRD, $connect_app);
                    $no_pr = $itemx[1];
                } else $no_pr = "";
                $qDeletePODtl = "DELETE FROM detail_surat_pemesanan_medis WHERE no_pemesanan='" . $_POST["hdn_id"] . "' AND kode_brng='" . $itemx[0] . "' AND no_pr_ref='" . $no_pr . "'";
        ...
        foreach ($lstKeep as $item) {
            ...
            $qUpdatePODtl = "UPDATE detail_surat_pemesanan_medis SET jumlah=" . str_replace(".", "", $_POST["inp_jml_" . $item]) . ", h_pesan=" . str_replace(".", "", $_POST["inp_hpesan_" . $item]) . ", subtotal=" . $_POST["hdn_subtotal_" . $item] . ", dis=" . str_replace(",", ".", $_POST["inp_disc1_" . $item]) . ", dis2=" . str_replace(",", ".", $_POST["inp_disc2_" . $item]) . ", besardis=" . $_POST["hdn_ndisc_" . $item] . ", total=" . $_POST["hdn_total_" . $item] . " WHERE no_pemesanan='" . $_POST["hdn_id"] . "' AND kode_brng='" . $itemx[0] . "' AND no_pr_ref='" . $no_pr . "'";
        ...
        foreach ($lstAdd as $item) {
            $itemx = explode("_", $item);
            if (isset($itemx[1])) {
                $qUpdatePRDtlA = "UPDATE detail_pengajuan_barang_medis SET status='Disetujui' WHERE no_pengajuan='" . $itemx[1] . "' AND kode_brng='" . $itemx[0] . "'";
                ...
                if ($PRDtl["X"] == $PRDtl["Y"]) {
                    $qUpdatePRA = "UPDATE pengajuan_barang_medis SET status='Disetujui' WHERE no_pengajuan='" . $itemx[1] . "'";
                    $updatePRA = mysqli_query($connect_app, $qUpdatePRA);
                    saveToTracker($qUpdatePRA, $connect_app);
                }
                $no_pr = $itemx[1];
            } else $no_pr = "";
            $qInsertPODtl = "INSERT INTO detail_surat_pemesanan_medis (no_pemesanan, kode_brng, no_pr_ref, kode_sat, kode_satbesar, isi, jumlah, h_pesan, subtotal, dis, dis2, besardis, total, jumlah2, status) VALUES ('" . $_POST["hdn_id"] . "', '" . $itemx[0] . "', '" . $no_pr . "', '" . $_POST["hdn_sk_" . $item] . "', '" . $_POST["hdn_sb_" . $item] . "', '" . $_POST["hdn_fk_" . $item] . "', " . str_replace(".", "", $_POST["inp_jml_" . $item]) . ", " . str_replace(".", "", $_POST["inp_hpesan_" . $item]) . ", " . $_POST["hdn_subtotal_" . $item] . ", " . str_replace(",", ".", $_POST["inp_disc1_" . $item]) . ", " . str_replace(",", ".", $_POST["inp_disc2_" . $item]) . ", " . $_POST["hdn_ndisc_" . $item] . ", " . $_POST["hdn_total_" . $item] . ", " . str_replace(".", "", $_POST["inp_jml_" . $item]) . ", 'Baru')";
        ...
        if ($updatePO && $updatePRD && $updatePRDtlD && $updatePRA && $updatePRDtlA && $deletePODtl && $updatePODtl && $insertPODtl) {
            ...
            logError("edit_purchase_order_m.php", "updatePO:" . $updatePO . "|updatePRDtlD:" . $updatePRDtlD . "|updatePRD:" . $updatePRD . "|deletePODtl:" . $deletePODtl . "|updatePODtl:" . $updatePODtl . "|updatePRDtlA:" . $updatePRDtlA . "|updatePRA:" . $updatePRA . "|insertPODtl:" . $insertPODtl);
        ...
        $PO_no = $_GET["id"];
        $qGetPO = "SELECT SPM.kode_suplier, nama_suplier, tanggal, total1, potongan, total2, ppn, meterai, tagihan, status, catatan FROM surat_pemesanan_medis SPM INNER JOIN datasuplier S ON SPM.kode_suplier=S.kode_suplier WHERE no_pemesanan = '" . $PO_no . "'";
        $getPO = mysqli_query($connect_app, $qGetPO);
        $PO = mysqli_fetch_assoc($getPO);
        $qGetPODtl = "SELECT DSPM.kode_brng, nama_brng, no_pr_ref, CASE WHEN Length(no_pr_ref) THEN Concat(DSPM.kode_brng,'_',no_pr_ref) ELSE DSPM.kode_brng END AS kode, DSPM.kode_sat, CASE WHEN DPBM.jumlah THEN DPBM.jumlah ELSE '-' END AS jumlah_diajukan, SK.satuan AS satuan_kecil, DSPM.kode_satbesar, SB.satuan AS satuan_besar, DSPM.isi, DSPM.jumlah AS jumlah_dipesan, h_pesan, subtotal, dis, dis2, besardis, DSPM.total, IfNull(Sum(stok), 0) AS stok FROM detail_surat_pemesanan_medis DSPM INNER JOIN databarang B ON DSPM.kode_brng=B.kode_brng INNER JOIN kodesatuan SK ON DSPM.kode_sat=SK.kode_sat INNER JOIN kodesatuan SB ON DSPM.kode_satbesar=SB.kode_sat LEFT JOIN detail_pengajuan_barang_medis DPBM ON DSPM.kode_brng=DPBM.kode_brng AND DSPM.no_pr_ref=DPBM.no_pengajuan LEFT JOIN gudangbarang G On G.kode_brng=B.kode_brng WHERE no_pemesanan = '" . $PO_no . "' GROUP BY kode_brng, no_pr_ref ORDER BY kode_brng, no_pr_ref";
        ...
        while ($data = mysqli_fetch_assoc($getPODtl)) {
            ...
            // $lstPODtl .= "<tr id='tr_" . $data["kode"] . "'><td>" . $data["kode_brng"] . "</td><td>" . $data["nama_brng"] . "</td><td class='num-input'>" . $data["jumlah_diajukan"] . "</td><td>" . $data["satuan_kecil"] . "<input type='hidden' id='hdn_sk_" . $data["kode"] . "' name='hdn_sk_" . $data["kode"] . "' value='" . $data["kode_sat"] . "'></td><td class='num-input'>" . $data["isi"] . "<input type='hidden' id='hdn_fk_" . $data["kode"] . "' name='hdn_fk_" . $data["kode"] . "' value='" . $data["isi"] . "'></td><td><input type='text' id='inp_jml_" . $data["kode"] . "' name='inp_jml_" . $data["kode"] . "' class='form-control num-input' value='" . $data["jumlah_dipesan"] . "' onKeyUp='removeNonNumeric(&quot;inp_jml_" . $data["kode"] . "&quot;); recalculatePO()' maxlength='5' autocomplete='off' style='width: 60px'></td><td>" . $data["satuan_besar"] . "<input type='hidden' id='hdn_sb_" . $data["kode"] . "' name='hdn_sb_" . $data["kode"] . "' value='" . $data["kode_satbesar"] . "'></td><td class='num-input'><input type='text' id='inp_hpesan_" . $data["kode"] . "' name='inp_hpesan_" . $data["kode"] . "' class='form-control num-input' style='width: 100px' value='" . number_format($data["h_pesan"], 0, ",", ".") . "' onKeyUp='removeNonNumeric(&quot;inp_hpesan_" . $data["kode"] . "&quot;); recalculatePO()' autocomplete='off' maxlength='11'></td><td class='num-input'><span id='span_subtotal_" . $data["kode"] . "' name='span_subtotal_" . $data["kode"] . "'>" . number_format($data["subtotal"], 0, ",", ".") . "</span><input type='hidden' id='hdn_subtotal_" . $data["kode"] . "' name='hdn_subtotal_" . $data["kode"] . "' value='" . $data["subtotal"] . "'></td><td><input type='text' id='inp_disc1_" . $data["kode"] . "' name='inp_disc1_" . $data["kode"] . "' class='form-control num-input' value='" . $data["dis"] . "' maxlength='2' onKeyUp='removeNonNumeric(&quot;inp_disc1_" . $data["kode"] . "&quot;); recalculatePO()' autocomplete='off'></td><td><input type='text' id='inp_disc2_" . $data["kode"] . "' name='inp_disc2_" . $data["kode"] . "' class='form-control num-input' value='" . $data["dis2"] . "' maxlength='2' onKeyUp='removeNonNumeric(&quot;inp_disc2_" . $data["kode"] . "&quot;); recalculatePO()' autocomplete='off'></td><td class='num-input'><span id='span_ndisc_" . $data["kode"] . "' name='span_ndisc_" . $data["kode"] . "'>" . number_format($data["besardis"], 0, ",", ".") . "</span><input type='hidden' id='hdn_ndisc_" . $data["kode"] . "' name='hdn_ndisc_" . $data["kode"] . "' value='" . $data["besardis"] . "'></td><td class='num-input td-total'><span id='span_total_" . $data["kode"] . "' name='span_total_" . $data["kode"] . "'>" . number_format($data["total"], 0, ",", ".") . "</span><input type='hidden' id='hdn_total_" . $data["kode"] . "' name='hdn_total_" . $data["kode"] . "' value='" . $data["total"] . "'></td><td class='num-input'><a class='a-text' onClick='getStockDtl(&quot;" . $data["kode_brng"] . "&quot;, &quot;M&quot;)'>" . number_format($data["stok"], 0, ",", ".") . "</a></td><td>" . $data["no_pr_ref"] . "<input type='hidden' id='hdn_PR_" . $data["kode"] . "' name='hdn_PR_" . $data["kode"] . "' value='" . $data["no_pr_ref"] . "'></td><td><a class='btn btn-danger btn-sm' title='Hapus' onClick='if(confirm(&quot;Apakah Anda yakin ingin menghapus data ini?&quot;)) removeItem(&quot;" . $data["kode"] . "&quot;)'><i class='fa fa-trash'></i></a></td></tr>";
            $lstPODtl .= "<tr id='tr_" . $data["kode"] . "'><td>" . $data["kode_brng"] . "</td><td>" . $data["nama_brng"] . "</td><td>" . $data["satuan_kecil"] . "<input type='hidden' id='hdn_sk_" . $data["kode"] . "' name='hdn_sk_" . $data["kode"] . "' value='" . $data["kode_sat"] . "'></td><td>" . $data["satuan_besar"] . "<input type='hidden' id='hdn_sb_" . $data["kode"] . "' name='hdn_sb_" . $data["kode"] . "' value='" . $data["kode_satbesar"] . "'></td><td class='num-input'>" . $data["isi"] . "<input type='hidden' id='hdn_fk_" . $data["kode"] . "' name='hdn_fk_" . $data["kode"] . "' value='" . $data["isi"] . "'></td><td class='num-input'>" . ($data["jumlah_diajukan"] != "-" ? number_format($data["jumlah_diajukan"], 0, ",", ".") . " " . $data["satuan_besar"] : $data["jumlah_diajukan"]) . "</td><td style='width: 60px'><input type='text' id='inp_jml_" . $data["kode"] . "' name='inp_jml_" . $data["kode"] . "' class='form-control num-input' value='" . $data["jumlah_dipesan"] . "' onKeyUp='removeNonNumeric(&quot;inp_jml_" . $data["kode"] . "&quot;); recalculatePO()' maxlength='5' autocomplete='off'></td><td style='width: 100px'>" . $data["satuan_besar"] . "</td><td class='num-input'><input type='text' id='inp_hpesan_" . $data["kode"] . "' name='inp_hpesan_" . $data["kode"] . "' class='form-control num-input' style='width: 100px' value='" . number_format($data["h_pesan"], 0, ",", ".") . "' onKeyUp='removeNonNumeric(&quot;inp_hpesan_" . $data["kode"] . "&quot;); recalculatePO()' autocomplete='off' maxlength='11'></td><td class='num-input'><span id='span_subtotal_" . $data["kode"] . "' name='span_subtotal_" . $data["kode"] . "'>" . number_format($data["subtotal"], 0, ",", ".") . "</span><input type='hidden' id='hdn_subtotal_" . $data["kode"] . "' name='hdn_subtotal_" . $data["kode"] . "' value='" . $data["subtotal"] . "'></td><td><input type='text' id='inp_disc1_" . $data["kode"] . "' name='inp_disc1_" . $data["kode"] . "' class='form-control num-input' value='" . str_replace(".", ",", $data["dis"]) . "' maxlength='5' onKeyUp='removeNonNumeric(&quot;inp_disc1_" . $data["kode"] . "&quot;); recalculatePO()' autocomplete='off'></td><td><input type='text' id='inp_disc2_" . $data["kode"] . "' name='inp_disc2_" . $data["kode"] . "' class='form-control num-input' value='" . str_replace(".", ",", $data["dis2"]) . "' maxlength='5' onKeyUp='removeNonNumeric(&quot;inp_disc2_" . $data["kode"] . "&quot;); recalculatePO()' autocomplete='off'></td><td class='num-input'><span id='span_ndisc_" . $data["kode"] . "' name='span_ndisc_" . $data["kode"] . "'>" . number_format($data["besardis"], 0, ",", ".") . "</span><input type='hidden' id='hdn_ndisc_" . $data["kode"] . "' name='hdn_ndisc_" . $data["kode"] . "' value='" . $data["besardis"] . "'></td><td class='num-input td-total'><span id='span_total_" . $data["kode"] . "' name='span_total_" . $data["kode"] . "'>" . number_format($data["total"], 0, ",", ".") . "</span><input type='hidden' id='hdn_total_" . $data["kode"] . "' name='hdn_total_" . $data["kode"] . "' value='" . $data["total"] . "'></td><td class='num-input'>" . number_format($data["stok"], 0, ",", ".") . " " . $data["satuan_kecil"] . " <a class='btn btn-primary btn-sm' onClick='getStockDtl(&quot;" . $data["kode_brng"] . "&quot;, &quot;M&quot;)' title='Klik untuk menampilkan detail stok barang per bangsal'><i class='fa fa-eye'></i></a></td><td>" . $data["no_pr_ref"] . "<input type='hidden' id='hdn_PR_" . $data["kode"] . "' name='hdn_PR_" . $data["kode"] . "' value='" . $data["no_pr_ref"] . "'></td><td><a class='btn btn-danger btn-sm' title='Hapus' onClick='if(confirm(&quot;Apakah Anda yakin ingin menghapus data ini?&quot;)) removeItem(&quot;" . $data["kode"] . "&quot;)'><i class='fa fa-trash'></i></a></td></tr>";
        }
    }
?>
...
<form name="frmEditPO" method="POST" target="" onSubmit="return validateForm()">
    <table id="tbl_editPO" class="table">
        <tr>
            ...
            <td style="width: 350px"><input type="text" id="inp_supplier" name="inp_supplier" class="form-control inline-input" style="width: 400px" onKeyUp="getSupplier()" value="<?php echo $PO["nama_suplier"] ?>" autocomplete="off"><input type="hidden" id="hdn_supplier" name="hdn_supplier" value="<?php echo $PO["kode_suplier"] ?>"></td>
        </tr>
        <tr>
            <td>Petugas</td>
        ...
        <tr>
            <td></td>
            <td colspan="3">
                Subtotal: <span id="span_total1" class="span-amount"><?php echo number_format($PO["total1"], 0, ",", ".") ?></span><input type="hidden" id="hdn_total1" name="hdn_total1" value="<?php echo $PO["total1"] ?>"><span class="inline-spacer">|</span>
                ...
                Total: <span id="span_total2" class="span-amount"><?php echo number_format($PO["total2"], 0, ",", ".") ?></span><input type="hidden" id="hdn_total2" name="hdn_total2" value="<?php echo $PO["total2"] ?>"><span class="inline-spacer">|</span>
                PPN (%) <input type="text" id="inp_ppn" name="inp_ppn" class="form-control inline-input num-input" value="<?php echo floor($PO["ppn"]/$PO["total2"]*100) ?>" maxlength="2" onKeyUp="removeNonNumeric('inp_ppn'); recalculatePO()" style="width: 40px" autocomplete="off"><span id="span_ppn" class="span-amount"><?php echo number_format($PO["ppn"], 0, ",", ".") ?></span><input type="hidden" id="hdn_ppn" name="hdn_ppn" value="<?php echo $PO["ppn"] ?>"><span class="inline-spacer">|</span>
                Meterai <input type="text" id="inp_meterai" name="inp_meterai" class="form-control inline-input num-input" value="<?php echo number_format($PO["meterai"], 0, ",", ".") ?>" maxlength="6" onKeyUp="removeNonNumeric('inp_meterai'); recalculatePO()" style="width: 65px" autocomplete="off"><span class="inline-spacer">|</span>
...
<script>
    ...
    setTimeout(function() { resizeDivScroll(); resizeSpacer(); recalculatePO(); }, 500);
</script>
