<?php
    ...
    $qGetPO = "SELECT no_pemesanan, tanggal, nama_suplier, P.nama, catatan, SPM.status, kadaluarsa FROM surat_pemesanan_medis SPM INNER JOIN datasuplier S ON SPM.kode_suplier=S.kode_suplier INNER JOIN pegawai P ON SPM.nip=P.nik WHERE tanggal BETWEEN '" . $today . "' AND '" . $today . "' order by no_pemesanan";
...
<a class="btn btn-primary" href="add_purchase_order_m.php"><i class="fa fa-plus"></i> Tambah</a>
...
<script>
    function getPOList() {
        ...
        } else ajax_getData("ajax_getdata.php?type=polist&supplier=" + supplier + "&start=" + mulai + "&end=" + selesai);
    }
    function togglePODetail(PO_no) {
        if ($("#tr_" + PO_no).css("display") == "none") {
            $("#chevron_" + PO_no).html("<i class='fa fa-chevron-circle-up'></i>");
            if ($("#td_" + PO_no).html() == "-") ajax_getData("ajax_getdata.php?type=podetail&id=" + PO_no);
        } else $("#chevron_" + PO_no).html("<i class='fa fa-chevron-circle-down'></i>");
        $("#tr_" + PO_no).fadeToggle();
    }
    function approvePO(PO_no) {
        ajax_getData("approve_po.php?id=" + PO_no + "&type=M");
    }
    function printPO(PO_no) {
        param = btoa("id=" + PO_no);
        window.open("show_report.php?id=purchase_order&param=" + param, "_blank", "width=1000,height=700");
    }
    setTimeout(function() {
        $(".tanggal").datepicker({
            autoclose: true,
