<?php
    ...
    if (isset($_POST["form_submit"])) {
        $barang = $_POST["hdn_itemsToPO"];
        $qGetSelPRItems = "SELECT * FROM (SELECT Concat(B.kode_brng, '_', no_pengajuan) AS kode, no_pengajuan, B.kode_brng, nama_brng, SK.kode_sat, SK.satuan AS satuan_kecil, kode_satbesar, SB.satuan AS satuan_besar, isi, dasar*isi As harga_beli, DPBN.jumlah_disetujui, IfNull(Sum(G.stok), 0) AS stok FROM ipsrsbarang B INNER JOIN kodesatuan SK ON B.kode_sat=SK.kode_sat INNER JOIN kodesatuan SB ON B.kode_satbesar=SB.kode_sat INNER JOIN detail_pengajuan_barang_nonmedis DPBN ON B.kode_brng=DPBN.kode_brng LEFT JOIN gpi_gudangbarangipsrs G ON B.kode_brng=G.kode_brng GROUP BY B.kode_brng, kode) AS t1 WHERE kode IN ('" . str_replace(",", "','", $barang) . "') ORDER BY kode_brng, no_pengajuan";
        ...
        while ($data = mysqli_fetch_assoc($getSelPRItems)) {
            ...
            $total += $h_pesan;
            $selPRItems .= "<tr id='tr_" . $data["kode"] . "'><td>" . $data["kode_brng"] . "</td><td>" . addslashes($data["nama_brng"]) . "</td><td>" . $data["satuan_kecil"] . "<input type='hidden' id='hdn_sk_" . $data["kode"] . "' name='hdn_sk_" . $data["kode"] . "' value='" . $data["kode_sat"] . "'></td><td>" . $data["satuan_besar"] . "<input type='hidden' id='hdn_sb_" . $data["kode"] . "' name='hdn_sb_" . $data["kode"] . "' value='" . $data["kode_satbesar"] . "'></td><td class='num-input'>" . $data["isi"] . "<input type='hidden' id='hdn_isi_" . $data["kode"] . "' name='hdn_isi_" . $data["kode"] . "' value='" . number_format($data["isi"], 0, ",", ".") . "'></td><td class='num-input'>" . number_format($data["jumlah_disetujui"], 2, ",", ".") . " " . $data["satuan_besar"] . "</td><td style='width: 60px'><input type='text' id='inp_jml_" . $data["kode"] . "' name='inp_jml_" . $data["kode"] . "' class='form-control num-input' value='" . number_format($data["jumlah_disetujui"], 2, ",", ".") . "' style='width: 60px' onKeyUp='removeNonNumeric(&quot;inp_jml_" . $data["kode"] . "&quot;); recalculatePO(&quot;N&quot;)' maxlength='4' autocomplete='off'></td><td style='width: 100px'>" . $data["satuan_besar"] . "</td><td class='num-input'><input type='text' id='inp_hpesan_" . $data["kode"] . "' name='inp_hpesan_" . $data["kode"] . "' class='form-control num-input' style='width: 100px' value='" . number_format($data["harga_beli"], 0, ",", ".") . "' onKeyUp='removeNonNumeric(&quot;inp_hpesan_" . $data["kode"] . "&quot;); recalculatePO(&quot;N&quot;)' autocomplete='off' maxlength='11'></td><td class='num-input'><span id='span_subtotal_" . $data["kode"] . "' name='span_subtotal_" . $data["kode"] . "'>" . number_format($h_pesan, 0, ",", ".") . "</span><input type='hidden' id='hdn_subtotal_" . $data["kode"] . "' name='hdn_subtotal_" . $data["kode"] . "' value='" . $h_pesan . "'></td><td><input type='text' id='inp_disc1_" . $data["kode"] . "' name='inp_disc1_" . $data["kode"] . "' class='form-control num-input' value='0' maxlength='5' onKeyUp='removeNonNumeric(&quot;inp_disc1_" . $data["kode"] . "&quot;); recalculatePO(&quot;N&quot;)' autocomplete='off'></td><td><input type='text' id='inp_disc2_" . $data["kode"] . "' name='inp_disc2_" . $data["kode"] . "' class='form-control num-input' value='0' maxlength='5' onKeyUp='removeNonNumeric(&quot;inp_disc2_" . $data["kode"] . "&quot;); recalculatePO(&quot;N&quot;)' autocomplete='off'></td><td class='num-input'><span id='span_ndisc_" . $data["kode"] . "' name='span_ndisc_" . $data["kode"] . "'>0</span><input type='hidden' id='hdn_ndisc_" . $data["kode"] . "' name='hdn_ndisc_" . $data["kode"] . "'></td><td class='num-input td-total'><span id='span_total_" . $data["kode"] . "' name='span_total_" . $data["kode"] . "'>" . number_format($h_pesan, 0, ",", ".") . "</span><input type='hidden' id='hdn_total_" . $data["kode"] . "' name='hdn_total_" . $data["kode"] . "' value='" . $h_pesan . "'></td><td class='num-input'>" . number_format($data["stok"], 0, ",", ".") . " " . $data["satuan_kecil"] . " <a class='btn btn-primary btn-sm' onClick='getStockDtl(&quot;" . $data["kode_brng"] . "&quot;, &quot;M&quot;)' title='Klik untuk menampilkan detail stok barang per bangsal'><i class='fa fa-eye'></i></a></td><td>" . $data["no_pengajuan"] . "<input type='hidden' id='hdn_PR_" . $data["kode"] . "' name='hdn_PR_" . $data["kode"] . "' value='" . $data["no_pengajuan"] . "'></td><td><a class='btn btn-danger btn-sm' title='Hapus' href='#' onClick='if (confirm(&quot;Apakah Anda yakin ingin menghapus data ini?&quot;)) removeItem(&quot;" . $data["kode"] . "&quot;)'><i class='fa fa-trash'></i></a></td></tr>";
            $i++;
        }
        echo '<script>
            window.opener.$("#tbl_listItems tbody").append("' . $selPRItems . '");
            window.opener.appendItems("PO", "' . $barang . '");
            window.opener.appendItems("PR", "' . $barang . '");
            window.opener.recalculatePO("N");
            window.close();
        </script>';
        exit();
    }
    $qGetPR = "SELECT no_pengajuan, P.nama, tanggal_disetujui, status, keterangan FROM pengajuan_barang_nonmedis PBN INNER JOIN pegawai P ON PBN.nip=P.nik WHERE tanggal_disetujui BETWEEN '" . $today . "' AND '" . $today . "' AND PBN.status='Pengajuan' ORDER BY tanggal_disetujui DESC, no_pengajuan DESC";
...
<html>
    <head>
        <title>Pengajuan Barang Non Medis yang Belum Selesai | Aplikasi Utilitas Khanza GPI</title>
    ...
    <body onClick="removeSuggest()">
        <div class="content-wrapper">
            <h4>Pengajuan Barang Non Medis yang Belum Selesai</h4>
            <form id="frmSelectPR" action="" method="POST">
                Tanggal&nbsp;&nbsp;
    ...
    <script>
        ...
        function getPRList() {
            ...
            } else ajax_getData("ajax_getdata.php?type=prnmlist&mode=popup&start=" + mulai + "&end=" + selesai);
        }
        function togglePRDetail(PR_no) {
            if ($("#tr_" + PR_no).css("display") == "none") {
                $("#chevron_" + PR_no).html("<i class='fa fa-chevron-circle-up'></i>");
                if ($("#td_" + PR_no).html() == "-") ajax_getData("ajax_getdata.php?type=prnmdetail&id=" + PR_no + "&mode=popup&data=<?php echo $_GET["data"] ?>");
