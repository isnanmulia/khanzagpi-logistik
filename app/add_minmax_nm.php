<?php
    ...
    if (isset($_POST["form_submit"])) {
        ...
        foreach ($items as $itm) {
            $qMinMax = "INSERT INTO gpi_minmax_nonmedis VALUES ('" . $itm . "', '" . $_POST["hdn_unit"] . "', " . $_POST["inp_min_" . $itm] . ", " . $_POST["inp_max_" . $itm] . ")";
        ...
        if ($status) {
            ...
            logError("add_minmax_nm.php", "minMax:" . $minMax);
            mysqli_rollback($connect_app);
            $msg = "Gagal menambahkan Stok Minimal/Maksimal Per Unit";
        }
        mysqli_autocommit($connect_app, TRUE);
        echo "<script>alert('" . $msg . "'); window.location.href='minmax_nm.php?unit=" . $_POST["hdn_unit"] . "'</script>";
    }
...
<form name="frmAddMinMaxNM" method="POST" target="" onSubmit="return validateForm()">
    <h4 id="h4_unit" style="display: inline-block">Unit: <?php echo $unit . " [" . $_GET["unit"] . "]" ?></h4><input type="hidden" id="hdn_unit" name="hdn_unit" value="<?php echo $_GET["unit"] ?>">
    <span style="margin-left: 50px"><a class="btn btn-primary" href="#" onClick="addUnitItem()"><i class="fa fa-plus"></i> Tambah Barang</a><input type="hidden" id="form_submit" name="form_submit" value="1"><input type="hidden" id="hdn_MinMaxitems" name="hdn_MinMaxitems"> <a class="btn btn-primary" href="#" onClick="window.location.href='minmax_nm.php?unit=<?php echo $_GET["unit"] ?>'"><i class="fa fa-times"></i> Batal</a> <button type="submit" class="btn btn-primary"><i class='fa fa-save'></i> Simpan</button></span>
