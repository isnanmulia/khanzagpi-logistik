<?php
    ...
    if (isset($_POST["form_submit"])) {
        ...
        $qGetLastNo = "SELECT IfNull(Max(Convert(Right(no_pemesanan,3),signed)),0) AS lastNo FROM surat_pemesanan_non_medis WHERE tanggal='" . $today . "'";
        $getLastNo = mysqli_query($connect_app, $qGetLastNo);
        $lastNo = mysqli_fetch_assoc($getLastNo)["lastNo"] + 1;
        $noSPN = "SPN" . date("ymd") . str_pad($lastNo, 3, "0", STR_PAD_LEFT);
        $qInsertPO = "INSERT INTO surat_pemesanan_non_medis (no_pemesanan, kode_suplier, nip, tanggal, subtotal, potongan, total, ppn, meterai, tagihan, status, catatan) VALUES ('" . $noSPN . "', '" . $_POST["hdn_supplier"] . "', '" . $_POST["hdn_user"] . "', '" . $today . "', " . $_POST["hdn_total1"] . ", " . $_POST["hdn_potongan"] . ", " . $_POST["hdn_total2"] . ", " . $_POST["hdn_ppn"] . ", " . str_replace(".", "", $_POST["inp_meterai"]) . ", " . $_POST["hdn_jml_tagihan"] . ", 'Baru', '" . $_POST["inp_catatan"] . "')";
        ...
        foreach ($items as $item) {
            ...
            if (strpos($item, "_") !== FALSE) {
                ...
                $qUpdatePRDtl = "UPDATE detail_pengajuan_barang_nonmedis SET status='Disetujui' WHERE no_pengajuan='" . $itemx[1] . "' AND kode_brng='" . $itemx[0] . "'";
                ...
                if ($PRDtl["X"] == $PRDtl["Y"]) {
                    $qUpdatePR = "UPDATE pengajuan_barang_nonmedis SET status='Disetujui' WHERE no_pengajuan='" . $itemx[1] . "'";
            ...
            $qInsertPODtl = "INSERT INTO detail_surat_pemesanan_non_medis (no_pemesanan, kode_brng, no_pr_ref, kode_sat, kode_satbesar, isi, jumlah, h_pesan, subtotal, dis, dis2, besardis, total, status) VALUES ('" . $noSPN . "', '" . $item . "', '" . $noPR . "', '" . $_POST["hdn_sk_" . $kode] . "', '" . $_POST["hdn_sb_" . $kode] . "', '" . $_POST["hdn_isi_" . $kode] . "', " . str_replace(",", ".", str_replace(".", "", $_POST["inp_jml_" . $kode])) . ", " . str_replace(".", "", $_POST["inp_hpesan_" . $kode]) . ", " . $_POST["hdn_subtotal_" . $kode] . ", " . str_replace(",", ".", $_POST["inp_disc1_" . $kode]) . ", " . str_replace(",", ".", $_POST["inp_disc2_" . $kode]) . ", " . $_POST["hdn_ndisc_" . $kode] . ", " . $_POST["hdn_total_" . $kode] . ", 'Baru')";
        ...
        if ($status) {
            ...
            logError("add_purchase_order_nm.php", "insertPO:" . $insertPO . "|updatePRDtl:" . $updatePRDtl . "|updatePR:" . $updatePR . "|insertPODtl:" . $insertPODtl);
...
<form name="frmAddPO" method="POST" target="" onSubmit="return validateForm()">
    <table id="tbl_addPO" class="table">
        <tr>
            ...
            <td style="width: 350px">SPNYYMMDDXXX</td>
            <td style="width: 150px">Supplier <span class="span-required">*</span></td>
            <td style="width: 350px"><input type="text" id="inp_supplier" name="inp_supplier" class="form-control inline-input" style="width: 400px" onKeyUp="getSupplierNM()" autocomplete="off"><input type="hidden" id="hdn_supplier" name="hdn_supplier"></td>
        </tr>
        <tr>
            <td>Petugas</td>
            <td>
        ...
        <tr>
            ...
            <td colspan="3">
                ...
                PPN (%) <input type="text" id="inp_ppn" name="inp_ppn" class="form-control inline-input num-input" value="10" maxlength="2" onKeyUp="removeNonNumeric('inp_ppn'); recalculatePO(&quot;N&quot;)" style="width: 40px" autocomplete="off"><span id="span_ppn" class="span-amount">0</span><input type="hidden" id="hdn_ppn" name="hdn_ppn"><span class="inline-spacer">|</span>
                Meterai <input type="text" id="inp_meterai" name="inp_meterai" class="form-control inline-input num-input" value="0" maxlength="6" onKeyUp="removeNonNumeric('inp_meterai'); recalculatePO(&quot;N&quot;)" style="width: 65px" autocomplete="off"><span class="inline-spacer">|</span>
